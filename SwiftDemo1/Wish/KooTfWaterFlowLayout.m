//
//  KooTfWaterFlowLayout.m
//  collectionView
//
//  Created by liyaqin on 2023/12/5.

#import "KooTfWaterFlowLayout.h"

/** 默认的列数*/
static const NSInteger KooTfDefaultColumeCount = 2;
/** 默认的行数*/
static const NSInteger KooTfDefaultRowCount = 5;

/** 每一列之间的间距*/
static const NSInteger KooTfDefaultColumeMargin = 10;
/** 每一行之间的间距*/
static const CGFloat KooTfDefaultRowMargin = 10;
/** 边缘之间的间距*/
static const UIEdgeInsets KooTfDefaultEdgeInset = {10, 10, 10, 10};

///** 每一行之间的间距*/
//static const CGSize KooTfDefaultHeaderSize = CGSizeMake(0, 66);
///** 每一行之间的间距*/
//static const CGSize KooTfDefaultFooterSize = CGSizeMake(0, 66);


@interface KooTfWaterFlowLayout ()

/** 存放所有cell的布局属性*/
@property (nonatomic, strong, nullable) NSMutableArray *attrsArray;

/** 存放每一列的最大y值*/
@property (nonatomic, strong, nullable) NSMutableArray *columnHeights;
/** 存放每一行的最大x值*/
@property (nonatomic, strong, nullable) NSMutableArray *rowWidths;
...
-(CGFloat)rowMarginWithIndexpath:(NSIndexPath *)indexPath;

/** 每一列之间的间距*/
-(CGFloat)columnMarginWithIndexpath:(NSIndexPath *)indexPath;
...
@implementation KooTfWaterFlowLayout

#pragma mark item属性配置

-(CGFloat)columnMarginWithIndexpath:(NSIndexPath *)indexPath
{
    if ([self.delegate respondsToSelector:@selector(columnMarginInWaterFlowLayout:section:)]) {
        return [self.delegate columnMarginInWaterFlowLayout:self section:indexPath.section];
    } else {
        return  KooTfDefaultColumeMargin;
    }
}

-(CGFloat)rowMarginWithIndexpath:(NSIndexPath *)indexPath
{
    if ([self.delegate respondsToSelector:@selector(rowMarginInWaterFlowLayout:section:)]) {
        return [self.delegate rowMarginInWaterFlowLayout:self section:indexPath.section];
    } else {
        return KooTfDefaultRowMargin;
    }
}
...
-(NSInteger)columnCount
{
    if ([self.delegate respondsToSelector:@selector(columnCountInWaterFlowLayout:)]) {
        ...
        return  KooTfDefaultColumeCount;
    }
}
...
-(void)prepareLayout {
    ...
    if (self.flowLayoutStyle == KooTfWaterFlowVerticalEqualWidth) {
    ...
    }else if (self.flowLayoutStyle == KooTfWaterFlowHorizontalEqualHeight){
    ...
    }else if (self.flowLayoutStyle == KooTfWaterFlowVerticalEqualHeight || self.flowLayoutStyle == KooTfLineWaterFlow){
    ...
    }else if(self.flowLayoutStyle == KooTfWaterFlowHorizontalGrid){
...
-(UICollectionViewLayoutAttributes *)layoutAttributesForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    }else if(self.flowLayoutStyle == KooTfWaterFlowVerticalEqualHeight){
        
        attrs.frame = [self itemFrameOfVHWaterFlow:indexPath];
        
    }else if(self.flowLayoutStyle == KooTfLineWaterFlow){
...
/*
 ...
 {
 if (self.flowLayoutStyle == KooTfLineWaterFlow) {
...
- (CGRect)itemFrameOfVerticalWaterFlow:(NSIndexPath *)indexPath{
    ...
    CGFloat collectionW = self.collectionView.frame.size.width;
    //设置布局属性item的frame
    CGFloat w = (collectionW - self.edgeInsets.left - self.edgeInsets.right - (self.columnCount - 1) * [self columnMarginWithIndexpath:indexPath]) / self.columnCount;
    CGFloat h = [self.delegate waterFlowLayout:self sizeForItemAtIndexPath:indexPath].height;
    
    //找出高度最短的那一列
    NSInteger destColumn = 0;
    
    if (self.enableLeftFirst) {
        CGFloat minColumnHeight = [self.columnHeights[indexPath.item % self.columnCount] doubleValue];

        destColumn = indexPath.item % self.columnCount;
        
        CGFloat x = self.edgeInsets.left + destColumn * (w + [self columnMarginWithIndexpath:indexPath]);
        CGFloat y = minColumnHeight;
        if (y != self.edgeInsets.top) {
            y += [self rowMarginWithIndexpath:indexPath];
        }
        ...
        if (self.maxColumnHeight < columnHeight) {
            ...
            [self.columnHeights replaceObjectAtIndex:(indexPath.item % self.columnCount) withObject:@(self.maxColumnHeight)];
        }
...
- (CGRect)itemFrameOfVHWaterFlow:(NSIndexPath *)indexPath{
    ...
    if (collectionW - [[self.rowWidths firstObject] floatValue] > w + self.edgeInsets.right) {
        ...
        x = [[self.rowWidths firstObject] floatValue] == self.edgeInsets.left  ? self.edgeInsets.left : [[self.rowWidths firstObject] floatValue] + [self columnMarginWithIndexpath:indexPath];
        if ([[self.columnHeights firstObject] floatValue] == self.edgeInsets.top) {
            ...
            y =  self.edgeInsets.top + headViewSize.height + [self rowMarginWithIndexpath:indexPath];
        }else{
        ...
        y = [[self.columnHeights firstObject] floatValue] + [self rowMarginWithIndexpath:indexPath];
        [self.rowWidths replaceObjectAtIndex:0 withObject:@(x + w)];
        [self.columnHeights replaceObjectAtIndex:0 withObject:@(y + h)];
...
- (CGRect)itemFrameOfHorizontalWaterFlow:(NSIndexPath *)indexPath{
    ...
    //设置布局属性item的frame
    CGFloat h = (collectionH - self.edgeInsets.top - self.edgeInsets.bottom - (self.rowCount - 1) * [self rowMarginWithIndexpath:indexPath]) / self.rowCount;
    CGFloat w = [self.delegate waterFlowLayout:self sizeForItemAtIndexPath:indexPath].width;
    ...
    CGFloat y = self.edgeInsets.top + destRow * (h + [self rowMarginWithIndexpath:indexPath]);
    CGFloat x = minRowWidth;
    if (x != self.edgeInsets.left) {
        x += [self columnMarginWithIndexpath:indexPath];
    }
...
- (CGRect)itemFrameOfHorizontalGridWaterFlow:(NSIndexPath *)indexPath{
    ...
    y = destRow == 0 ? self.edgeInsets.top : self.edgeInsets.top + h + [self rowMarginWithIndexpath:indexPath];
    
    x = [self.rowWidths[destRow] doubleValue] == self.edgeInsets.left ? self.edgeInsets.left : [self.rowWidths[destRow] doubleValue] + [self columnMarginWithIndexpath:indexPath];
    //更新最短那行的宽度
    if (h >= collectionH - self.edgeInsets.bottom - self.edgeInsets.top) {
        x = [self.rowWidths[destRow] doubleValue] == self.edgeInsets.left ? self.edgeInsets.left : self.maxRowWidth + [self columnMarginWithIndexpath:indexPath];
        for (NSInteger i = 0; i < 2; i++) {
...
- (CGRect)headerViewFrameOfVerticalWaterFlow:(NSIndexPath *)indexPath{
    ...
    if (self.flowLayoutStyle == KooTfWaterFlowVerticalEqualWidth) {
        ...
        if (![self.delegate respondsToSelector:@selector(waterFlowLayout:sizeForFooterViewInSection:)] || [self.delegate waterFlowLayout:self sizeForFooterViewInSection:indexPath.section].height == 0) {
            y = self.maxColumnHeight == 0 ? self.edgeInsets.top : self.maxColumnHeight + [self rowMarginWithIndexpath:indexPath];
        }
...
- (CGRect)footerViewFrameOfVerticalWaterFlow:(NSIndexPath *)indexPath{
    ...
    if (self.flowLayoutStyle == KooTfWaterFlowVerticalEqualWidth ) {
        ...
        CGFloat x = 0;
        CGFloat y = size.height == 0 ? self.maxColumnHeight : self.maxColumnHeight + [self rowMarginWithIndexpath:indexPath];
